        ...
        <div className="flex gap-6 justify-start flex-wrap max-[640px]:gap-4">
          {buttons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={button.text ? "hero-contact-link transition-colors duration-200 underline" : "icon-link"}
              target={button.href.startsWith('http') ? '_blank' : undefined}
              rel={button.href.startsWith('http') ? 'noopener noreferrer' : undefined}
            >
              {button.text ? (
                <>
                  <button.icon size={20} className="inline mr-3 max-[640px]:w-4 max-[640px]:h-4 max-[640px]:mr-2" />
