...
function SocialIcon({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
    ...
    return (
        <div className="relative">
            <div className={`absolute top-full mt-3 left-1/2 -translate-x-1/2 bg-neutral-800 text-white text-sm px-2.5 py-1 rounded whitespace-nowrap transition-all ease-out ${hovered ? "duration-300" : "duration-150"} pointer-events-none ${hovered ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}>
                {label}
            </div>
            <a
                href={href}
